...
class TestConfigFiles:
    ...
    def test_profiles_exist(self):
        for name in ["pa", "coder", "fnb", "founder"]:
            assert os.path.isdir(os.path.join(REPO_ROOT, "profiles", name)), f"profiles/{name} missing"

    def test_skills_exist(self):
        for name in ["fnb-expert", "demand-forecasting", "entrepreneur-frameworks"]:
...
class TestProfileConfigs:
    @pytest.mark.parametrize("profile", ["pa", "coder", "fnb", "founder"])
    ...
    def test_coder_profile_high_reasoning(self):
        cfg = _load_yaml("profiles/coder/config.yaml")
        assert cfg.get("agent", {}).get("reasoning_effort") == "high"

    def test_founder_profile_high_reasoning(self):
        cfg = _load_yaml("profiles/founder/config.yaml")
        assert cfg.get("agent", {}).get("reasoning_effort") == "high"
...
class TestDockerfile:
    ...
    def test_gh_cli_installed(self, dockerfile):
        assert "gh_" in dockerfile and "gh" in dockerfile, \
            "GitHub CLI must be installed in Dockerfile"
